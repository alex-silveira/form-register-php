...
class Validacao{
    ...
    public static function validarSenha($password, $cPassword, $page, $key){

        if(empty($password)){
            $msg = "O campo de senha é obrigatório";
            utf8_encode($msg);
            header("Location: ../index.php?key=$key&page=$page&msg=$msg");
            return false;
        }
            ...
            utf8_encode($msg);
            header("Location: ../index.php?page=$page&msg=$msg");
            return false;
        }
    ...
    public static function validarCapatcha($captcha, $result, $page, $key){
	    if(empty($captcha)){
    ...
    public static function validarKey($key){
        if(empty($key)){
            $msg = "Chave em branco não é permitida";
            utf8_encode($msg);
            header("Location: ../index.php?key=$key&page=4&msg=$msg");
            return false;
        }
        else if(!preg_match("/^([a-zA-Z0-9]+)$/",$key)){
            $msg = "A chave deve conter somente letras e números";
            utf8_encode($msg);
            header("Location: ../index.php?key=$key&page=4&msg=$msg");
            return false;
        }
